...
class OuterCalss {
	...
	// 비정적 내부 클래스 - 인스턴스 필드, 인스턴스 메서드 처럼 사용(외부 클래스를 인스턴스화 해야함)
	class InnerClass {
		void display() {
	...
	static class StaticClass {
		void display() {
			// System.out.println("외부 클래스 필드에 접근(인스턴스): " + outerField); - 인스턴스를 해야 접근이 가능하기 때문에 static 필드로는 접근이 불가능(오류방지?)
...
public class A_Inner01 {
	public static void main(String[] args) {
		// === 비정적 내부 클래스 ===
		// 1) 외부 클래스 인스턴스화
		OuterCalss outer1 = new OuterCalss();
		
		// 2) 외부 클래스.내부 클래스 형식으로 내부 클래스를 생성
		// 외부클래스타입.내부클래스타입 변수 = 외부인스턴스.new 내부클래스();
		OuterCalss.InnerClass inner = outer1.new InnerClass();
		inner.display(); // 외부 클래스 필드에 접근: 외부 클래스의 필드
		
		// === 정적 내부 클래스 (스태틱 클래스***) ===
		// 1) 외부 클래스 인스턴스화가 필요없이 클래스를 통한 내부 클래스 생성이 가능
		// 외부클래스타입.내부클래스타입 변수 = new 외부클래스.내부클래스();
		OuterCalss.StaticClass staticInner = new OuterCalss.StaticClass();
		
		// === 메서드 내부 클래스(지역 클래스) ===
		OuterCalss outer2 = new OuterCalss();
		outer2.outerMethod(); // 로컬 변수에 접근로컬 변수
